        ...
        <div className="flex flex-wrap gap-3">
          {actionButtons.map(action => (
            <button
              ...
              type="button"
              className={cn(
                'rounded-full px-5 py-2 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-400',
  ...
  return (
    <Card>
      <header className="flex flex-wrap items-center justify-between gap-4">
        ...
        <button type="button" className="inline-flex items-center gap-1 rounded-full bg-slate-900 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-slate-900/10 transition hover:bg-slate-800 dark:bg-white dark:text-slate-900">
        ...
        {taskSections.map(section => (
          <div key={section.id} className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <button type="button" className="text-sm font-semibold text-slate-500 transition hover:text-slate-900 dark:text-slate-400 dark:hover:text-white">
                View all
              </button>
            </div>
            <div className="space-y-3">
              {section.tasks.map(task => (
                <article
                  key={task.id}
                  className="flex flex-col gap-3 rounded-2xl border border-slate-100 bg-white/80 p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg dark:border-slate-800 dark:bg-slate-900/60"
                >
                  <div>
  ...
  return (
    <Card>
      <div className="flex items-center justify-between">
        ...
        <button type="button" className="text-sm font-semibold text-purple-600 transition hover:text-purple-500 dark:text-purple-300">
          View details
        </button>
      </div>
  ...
  return (
    <Card className="bg-linear-to-br from-purple-500 via-indigo-500 to-sky-500 text-white shadow-purple-500/30 dark:shadow-purple-900/30">
      <div className="space-y-4">
        ...
        <button type="button" className="inline-flex items-center rounded-full bg-white/20 px-5 py-2 text-sm font-semibold backdrop-blur transition hover:bg-white/30">
          + Invite people
        </button>
      </div>
    </Card>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        {projects.map(project => (
          <button
            type="button"
            key={project.id}
            className="flex flex-col gap-3 rounded-2xl border border-slate-100 bg-white/80 p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg dark:border-slate-800 dark:bg-slate-900/60"
          >
            <span
              className={cn(
                'inline-flex h-10 w-10 items-center justify-center rounded-2xl text-lg font-semibold',
                project.highlight,
              )}
            >
              {project.icon}
...
function CalendarCard() {
  return (
    <Card>
      ...
      <div className="mt-6 flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-500 dark:bg-slate-800/60 dark:text-slate-300">
      ...
      <div className="mt-6 rounded-2xl border border-slate-100 bg-white/80 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/60">
        ...
        <div className="mt-4 flex items-center justify-between text-sm text-slate-500 dark:text-slate-400">
          ...
          <span className="inline-flex items-center justify-center rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600 dark:bg-slate-800 dark:text-slate-200">
            +2
          </span>
        </div>
      </div>
    ...
    <Card>
      ...
      <div className="mt-6 space-y-4">
        {reminders.map(reminder => (
          <div
            key={reminder.id}
            className="rounded-2xl border border-slate-100 bg-white/80 p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg dark:border-slate-800 dark:bg-slate-900/60"
          >
            <p className="font-semibold text-slate-900 dark:text-white">
              {reminder.title}
            </p>
